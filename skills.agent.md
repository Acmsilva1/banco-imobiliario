...
- **Stack Preferencial:** Node.js, React, Tailwind CSS (Bento UI style); **neste repo** API **Fastify** + **Supabase** (Postgres gerido).
...
2. 🏗️ Architecture: feature slices + core (Banco imobiliário) `<a name="architecture"></a>`
Monorepo **`web/`** + **`api/`** (Fastify + TypeScript):
- **Front:** `web/src/features/{bank,lobby}/`, `web/src/shared/`; Vite **5181** `strictPort`; entrada em `web/src/main.tsx` (ou equivalente).
- **API:** `api/src/features/bank/` (regras, serviços, tipos); `api/src/core/` (cliente Supabase, config); `api/src/index.ts`.
- **Dados:** Supabase via `api/src/core/supabase.ts`; sem regra de jogo no cliente além da UI.
- **Fluxo:** rotas Fastify → serviços em `features/bank/`; testes colados às regras (`*.test.ts`).
- **Isolamento:** `bank` vs `lobby` no front; API centrada em domínio do jogo em `api/src/features/bank/`.
...
- **Este repositório:** **Fastify** em `api/src/index.ts` + Socket.io conforme dependências — não documentar como Express.
...
2.1 Onde está o código (repositório **banco-imobiliario**)

| Âmbito | Árvore típica | O que documentar aqui |

|--------|----------------|------------------------|

| **[B]** | `api/src/features/bank/` | Regras do jogo, serviços, tipos, testes Vitest. |

| **[B]** | `api/src/core/supabase.ts`, `api/src/index.ts` | Fastify, registo de rotas, cliente Supabase. |

| **[F]** | `web/src/features/bank/`, `web/src/features/lobby/` | Salas, tabuleiro, UI do jogo. |

| **[F]** | `web/src/shared/` | Componentes e helpers partilhados. |

| **[T]** | Contrato **HTTP** (REST do jogo) | **## 4** + **## 3**; inventário em **## 4**. |

| **[T]** | **Estado sensível** (salas, jogadores) | **## 6** — não expor IDs internos desnecessários. |
...
- Conteúdo **servidor** (`api/src/`, Fastify, Supabase): **## 4**, **## 5** e **## 7**.
...
Core (`api/src/core/`): Supabase e arranque Fastify.

Features (`api/src/features/bank/`): regras e persistência do jogo.

Fluxo: plugin/rota Fastify → `bank.service` / `bank.rules`.

Regra de Ouro: regras de negócio testáveis em `.test.ts`; sem duplicar regras no `web/`.

3.2 Camada Frontend **[F]** (`web/src/`)

Shared (`web/src/shared/`): UI comum.

Features (`web/src/features/bank/`, `lobby/`): fluxo de sala e tabuleiro.

Nomenclatura: PascalCase / camelCase. Ver `agents.md`, `documento.md`, `mudanca_arquitetura.md`.

Dev: web **5181**; API na porta definida no `package.json` da raiz / `api/`.
...
Controle de Acesso **[B]**: validação nas rotas Fastify; chaves Supabase só no servidor.

Controle de Acesso **[F]**: não persistir tokens em `localStorage` sem decisão documentada em **## 6**.

Privacidade (LGPD) **[T]**: dados de perfil em jogo multi-jogador — minimizar PII nas respostas.

Integridade **[B]**: mudanças em schema Supabase / RLS acompanhadas de SQL ou migration no processo do repo.
...
Ponto(s) de entrada (`web/src/main.tsx`, rotas/features `bank`/`lobby`), estado, `web/src/shared`, chamadas REST ao Fastify. Se não houver UI: **«Não aplicável.»** + uma linha.

**## 4. Backend, API e processamento [B]**

Rotas **Fastify**, registradores, handlers e serviços em `api/src/`. Tabela obrigatória **inventário de API**: **ID da superfície** (mesmo da secção 2) ou **«Módulo»** | **Método e caminho** | **Observação** (REST, Socket.io, etc.). **Mínimo uma linha**. Sem caminhos `web/src/` nesta secção.

**## 5. Persistência, dados e consultas [B]**

Supabase/Postgres (schema remoto); tabela **mapeamento de dados** (área da UI ou ID | entrada | ficheiro `api/src/` ou `web/src/` | tabela/policy Supabase). Incluir **SQL no browser** nesta tabela quando existir. Se não houver persistência: **«Não aplicável.»** + uma linha.
...
Lista formal de melhorias, riscos, dívidas técnicas; última linha com **DOC-ID** da revisão do documento (ex.: «Documento revisado em … — `BANCO-IMOB-…-TEC-RNN`.»). Se não houver observações: **«Nenhuma observação adicional nesta revisão.»**
...
Utilizar o prefixo acordado para o produto (ex.: `BANCO-IMOB-{SIGLA}-TEC-RNN` ou o padrão em `documento.md` / identificação do módulo).
